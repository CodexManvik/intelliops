...
const SEV_INCIDENT = {
  critical: { bg: "#FEF2F2", text: "#DC2626", border: "#FECACA", label: "CRITICAL", left: "#EF4444" },
  high:     { bg: "#FFFBEB", text: "#D97706", border: "#FDE68A", label: "HIGH",     left: "#F59E0B" },
  warning:  { bg: "#FEFCE8", text: "#CA8A04", border: "#FEF08A", label: "WARNING",  left: "#EAB308" },
};

const STAT_INCIDENT = {
  investigating: { bg: "#EEF2FF", text: "#4338CA", label: "Investigating" },
  identified:    { bg: "#FFFBEB", text: "#D97706", label: "Identified"    },
  resolved:      { bg: "#F0FDF4", text: "#16A34A", label: "Resolved"      },
};

function IncidentReports({ onViewIncident }: { onViewIncident: (inc: Incident) => void }) {
  return (
    <section id="incidents" style={{ background: "#FFFFFF", padding: "96px 0", borderTop: "1px solid #E4E5E9" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 32px" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 40, flexWrap: "wrap", gap: 16 }}>
          <div>
            <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 12, color: "#2F5FF6", fontWeight: 500, marginBottom: 10, letterSpacing: "0.04em" }}>
              INCIDENT REPORTS
            </div>
            <h2 style={{ fontSize: "clamp(26px, 3.5vw, 36px)", fontWeight: 800, color: "#101114", letterSpacing: "-0.03em", lineHeight: 1.15, margin: "0 0 12px" }}>
              Every incident. Full AI context.
            </h2>
            <p style={{ fontSize: 15, color: "#6B7280", lineHeight: 1.6, margin: 0, maxWidth: 480 }}>
              IntelliOps auto-creates structured incident reports with root-cause summaries, event timelines, and affected metrics — from first anomaly to resolution.
            </p>
          </div>
          <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: "#9CA3AF" }}>
            {INCIDENTS.filter(i => i.status !== "resolved").length} active · {INCIDENTS.filter(i => i.status === "resolved").length} resolved today
          </div>
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {INCIDENTS.map(inc => {
            const s = SEV_INCIDENT[inc.severity];
            const st = STAT_INCIDENT[inc.status];
            return (
              <div key={inc.id}
                onClick={() => onViewIncident(inc)}
                style={{
                  background: "#FAFAFA", border: "1px solid #E4E5E9",
                ...
                onMouseEnter={e => {
                  (e.currentTarget as HTMLElement).style.boxShadow = "0 4px 16px rgba(47,95,246,0.08)";
                  (e.currentTarget as HTMLElement).style.borderColor = `${s.left}`;
                ...
                onMouseLeave={e => {
                  ...
                  (e.currentTarget as HTMLElement).style.borderLeftColor = s.left;
                }}
              >
                {/* Severity */}
                <div style={{
                  ...
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: s.left }} />
                  <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 10, fontWeight: 700, color: s.text }}>
                    {s.label}
                  </span>
                </div>
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 2 }}>
                    <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: "#9CA3AF", flexShrink: 0 }}>{inc.id}</span>
                    <span style={{ fontSize: 14, fontWeight: 600, color: "#101114", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{inc.title}</span>
                  </div>
                  <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: "#6B7280" }}>service: {inc.service}</span>
                </div>

                {/* Start time */}
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: "#9CA3AF" }}>started</div>
                  <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: "#6B7280" }}>
                    {new Date(inc.startTime).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })} UTC
                  </div>
                </div>

                {/* Duration */}
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: "#9CA3AF" }}>duration</div>
                  <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: "#6B7280" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                  <div style={{
                    background: st.bg, borderRadius: 6, padding: "3px 9px",
                    fontSize: 11, fontWeight: 600, color: st.text,
                  }}>{st.label}</div>
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                    <path d="M5 3l4 4-4 4" stroke="#9CA3AF" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
              </div>
            );
        ...
        <div style={{ marginTop: 20, textAlign: "center" }}>
          <a href="#" style={{
            ...
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
              <path d="M2.5 6h7M6 2.5l3.5 3.5L6 9.5" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </a>
        </div>
      </div>
